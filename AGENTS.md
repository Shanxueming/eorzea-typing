...
- 词序列(`createWordQueue(pool, seed)`)客户端和服务端各自独立生成,只靠
  ...
  校验的设计,改动前先想清楚这个前提。内部是一副持续到底的洗牌牌堆(发完
  整个 pool 才重新洗一次),**不要**再改回"按固定批次大小反复重新洗整个池子
  只留前 N 个"的写法——那样每一批都独立随机抽样,池子只要比批次大,同一个词
  跨批次几乎没有"不重复"的保证,无限模式这种长会话会明显感觉到某些词
  反复刷到(2026-08-06 修过一次)。
